...
//                    if (ctt < 0) console.log(ctt);
        ...
        var cyclesToRun = 0;
        var cyclesUntilNextIrq = 0;
        var ctt = 0;
        var missed = 0;
        this.polltime = function (cycles) {
            cyclesToRun += cycles | 0;
            if (cyclesToRun >= cyclesUntilNextIrq) {
                ctt = cyclesToRun;
                sync();
...
//                    console.log(missed);
        ...
        function updateCyclesUntilNextIrq() {
            cyclesUntilNextIrq = (self.regs[0] - self.hc) + 1;
            var interlaced = (self.regs[8] & 3) === 3; // todo rename ilSyncAndVideo as above?
            var linesPerVChar = interlaced ? (self.regs[9] >>> 1) : self.regs[9];
            var linesLeft = (self.regs[7] - self.vc) * linesPerVChar + (linesPerVChar - self.sc);
            if (self.hvblcount) linesLeft += self.hvblcount - 1;
            if (linesLeft >= 0) {
                cyclesUntilNextIrq += (linesLeft - 0) * (self.regs[0] + 1);
            }
            if (self.halfClock) cyclesUntilNextIrq *= 2;
        }
